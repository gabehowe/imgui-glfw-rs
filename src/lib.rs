...
use glfw::{Action, Key, Modifiers, MouseButton, PWindow, StandardCursor, WindowEvent};
...
impl imgui::ClipboardBackend for GlfwClipboardBackend {
    fn get(&mut self) -> Option<String> {
        ...
        Some(imgui::ImString::new(c_str.to_str().unwrap()).to_string())
    }

    fn set(&mut self, value: &str) {
        unsafe {
            glfw::ffi::glfwSetClipboardString(self.0 as *mut GLFWwindow, value.to_string().as_ptr() as *const c_char);
        };
    }
}
...
impl ImguiGLFW {
    ...
    pub fn new(imgui: &mut Context, window: &mut PWindow) -> Self {
        unsafe {
    ...
    pub fn frame(&'static mut self, window: &mut PWindow, imgui: &'static mut Context) -> &mut Ui {
        let io = imgui.io_mut();
    ...
    pub fn draw<'ui>(&mut self, ui: &mut Ui, window: &mut PWindow) {
        let io = ui.io();
        if !io
            .config_flags
